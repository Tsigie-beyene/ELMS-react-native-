...
const Message = ({ route }) => {
  ...
  const createNewMessage = async () => {
    ...
    try {
      const response = await apiInstance.post(
        `student/question-answer-create/`,
        json
      );
      console.log(response.data);
      setSelectedConversation(response.data.question);
          ...
          <>
            <ScrollView showsVerticalScrollIndicator={false} className="mt-5">
              ...
              <View className="flex-row justify-center items-center gap-3 mt-4">
                ...
                <TouchableOpacity
                  onPress={openRBSheetCreateMessage}
                  className="bg-[#280e49] w-100 p-2 rounded-md"
                >
              ...
              <Text className="font-bold text-[20px] mt-10 text-center mb-5">
                All Messages
              </Text>
      ...
      <View>
        ...
        >
          {rbSheetType === "messages" ? (
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View className="chat-input flex-row items-center justify-between p-4 border-t">
                    <TextInput
                      ...
                      onChangeText={(value) =>
                        handleCreateMessage("message", value)
                      }
                      type="text"
                      className="chat-input-field flex-1 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500"
                      placeholder="Type your message..."
                    />
                    <TouchableOpacity
                      onPress={sendNewMessage}
                      className="chat-send-button bg-blue-500 text-white px-4 py-2 ml-2 rounded-md hover:bg-blue-700"
                    >
                      <Text>Send</Text>
                    </TouchableOpacity>
                  </View>
            ...
            <>
              <Text className="text-center font-semibold text-[17px]">
                Create New Message
              </Text>

              <View className="chat-input p-4 mt-3">
                <TextInput
                  onChangeText={(value) => handleCreateMessage("title", value)}
                  value={createMessage.title}
                  multiline
                  type="text"
                  className="font-semibold chat-input-field flex-1 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500"
                ...
                <TextInput
                  ...
                  multiline
                  type="text"
                  className="font-normal mb-3 chat-input-field flex-1 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  placeholder="Question Content"
                />
                <TouchableOpacity
                  onPress={createNewMessage}
                  className="w-32 bg-blue-500 px-4 py-2 ml-2 rounded-md hover:bg-blue-700"
                >
