  ...
  const handleEditNote = async (noteId) => {
    ...
    try {
      ...
      const response = await apiInstance.patch(
        `student/course-note-detail/${user_id}/${enrollment_id}/${parseInt(
          noteId
        )}/`,
        json
      );
          ...
          <>
            <ScrollView showsVerticalScrollIndicator={false} className="mt-5">
              ...
              <View className="flex-row justify-center items-center gap-3 mt-4">
                ...
                <TouchableOpacity
                  onPress={() => openRBSheet("note-create", "")}
                  className="bg-[#280e49] w-100 p-2 rounded-md"
                >
              ...
              {notes?.map((n, index) => (
                <View key={index} className="bg-gray-200 p-3 rounded-md mb-4">
                  ...
                  <Text className="mt-3 mb-3">
                    Date: {moment(n?.date).format("DD MMM, YYYY")}
                  </Text>
                  <View className="flex flex-row gap-2">
                    <TouchableOpacity
                      onPress={() => openRBSheet("note-detail", n)}
                      className="bg-[#280e49] w-7 h-7 rounded-md flex justify-center items-center"
                    >
              ...
              {rbSheetType === "note-detail" ? (
                <>
                  ...
                  <View className="chat-input p-4 mt-3">
                    <TextInput
                      onChangeText={(value) => handleCreateNote("title", value)}
                    ...
                    />
                    <TextInput
                      onChangeText={(value) => handleCreateNote("note", value)}
                    ...
                    />
                    <TouchableOpacity
                      onPress={() => handleEditNote(selectedNote?.id)}
                      className="w-16 bg-blue-500 px-4 py-2 ml-2 rounded-md hover:bg-blue-700"
                    >
