...
const Reviews = ({ route }) => {
  ...
  const fetchReview = async () => {
    ...
    try {
      ...
      if (course) {
        const review_response = await apiInstance.get(
          `student/review/list/${course?.course?.id}/`
        );
    ...
    try {
      if (studentReview?.rating) {
        ...
        await apiInstance.patch(
          `student/review-detail/${user_id}/${studentReview?.id}/`,
          json
        );
          ...
          <>
            <ScrollView showsVerticalScrollIndicator={false} className="mt-5">
              ...
              <View className="bg-gray-200 p-3 rounded-md mb-4">
                ...
                <Text className="mt-7 mb-1">
                  By: {studentReview?.user?.username}
                </Text>
                <Text className="">
                  Date: {moment(studentReview?.date).format("DD MMM, YYYY")}
                </Text>
              ...
              <Text className="font-bold text-[20px] mt-7 text-center mb-5">
                All Course Review
              </Text>
          ...
          >
            <View style={{ flex: 1 }}>
              <View className="chat-input p-4 mt-3">
                <SelectList
                  defaultOption={default_data}
                  setSelected={(val) => setSelectedRating(val)}
                  data={data}
                  save="key"
                ...
                <TextInput
                  ...
                  defaultValue={studentReview?.review || "No Review"}
                  multiline
                  type="text"
                ...
                />
                <TouchableOpacity
                  onPress={handleUpdateReviewSubmit}
